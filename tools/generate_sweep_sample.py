...
clip: tools/prepare_clip.py sanitizes it, retargets its hands, audits it in
MuJoCo and files it under clips/. It has no runner of its own.
...
Larger hand motion is not authored here. It comes from a real bundle
sample, where the fingers move because a person moved them.
...
Safety by construction, then verified independently:
  - arm amplitudes are capped (--arm-amplitude) and clipped 10% inside the
    ...
    times the deploy screening rows (tools/sweep_joint_limits.yaml);
  - the verdict is still EARNED through tools/prepare_clip.py's dynamic
    audit, which replays the result in MuJoCo with the hands actuated and
    judges peak contact force and arm torque.

Run it from the repo root, then prepare and replay the result:
...
Operator commands: docs/replay.md.
"""
...
# tools/ sits one level below the repo root, the same convention
# prepare_clip.py and clip_audit.py use, so the defaults below resolve in the
# teleop container too (tools/ and RobotSTAR_demos/ are both mounted there).
REPO_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_DEMOS_ROOT = REPO_ROOT / 'RobotSTAR_demos'
# The generated sample is its own small bundle root, with its own manifest,
# so prepare_clip records a bundle_manifest_sha256 that identifies it.
DEFAULT_OUT_DIR = DEFAULT_DEMOS_ROOT / 'sweep-test'
DEFAULT_ARM_LIMITS = REPO_ROOT / 'tools' / 'sweep_joint_limits.yaml'
...
def main() -> int:
    ...
    ap.add_argument('--arm-limits', type=Path, default=None,
                    help=f'arm limits yaml (default {DEFAULT_ARM_LIMITS})')
    ap.add_argument('--out', type=Path, default=None,
                    help=f'sweep bundle root to write (default {DEFAULT_OUT_DIR})')
    ap.add_argument('--demos-root', type=Path, default=None,
                    help=f'bundle root holding the donor sample (default {DEFAULT_DEMOS_ROOT})')
    args = ap.parse_args()

    out_dir = args.out or DEFAULT_OUT_DIR
    demos_root = args.demos_root or DEFAULT_DEMOS_ROOT
    ...
    limits_path = args.arm_limits or DEFAULT_ARM_LIMITS
    ...
    # ---- write the sample ------------------------------------------------
    method_dir = out_dir / 'samples' / SAMPLE_NAME / 'GT'
    ...
    # Manifest over the three generated files. This folder is its own
    # bundle root, so prepare_clip hashes THIS file for the clip's
    # source.bundle_manifest_sha256 and the sweep clip is identifiable.
    manifest_lines = []
    for p in (npz_path, meta_path, kp_path):
        digest = hashlib.sha256(p.read_bytes()).hexdigest()
        manifest_lines.append(f'{digest}  {p.relative_to(out_dir)}')
