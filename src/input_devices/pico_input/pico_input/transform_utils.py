...
Single authoritative implementation of all coordinate/rotation transform
functions on the PICO path. pico_input_node and incremental_controller import
from here.

Config source: pico_input/config/robot_frames.yaml (single source of truth).

History: this was tianji_world_output.transform_utils. It moved into pico_input
unchanged when the Tianji arm packages were removed -- pico_input was its only
remaining consumer, and the math here converts PICO's frame into the world and
chest frames, which is an input-side concern independent of which arm output
consumes the resulting topics. No function body was modified in the move.
