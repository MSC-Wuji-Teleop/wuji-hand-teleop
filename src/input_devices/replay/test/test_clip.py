...
def test_parent_not_a_playable_directory_is_refused(tmp_path: Path):
    ...
    clips/rejected/ is where prepare_clip.py puts a clip the audit turned
    down, so it has to stay out of this set. ``home`` is in this list because
    clips/home/ was playable until the rehome was removed.
    """
    for parent in ("candidate", "rejected", "clips", "home", "homes", "Safe"):
        d = write_clip(tmp_path / parent / CLIP_NAME)
        with pytest.raises(ClipError, match="not under a directory named"):
            load_clip(d)


def test_safe_is_the_only_playable_parent(tmp_path: Path):
    assert PLAYABLE_PARENT_DIR_NAMES == ("safe",)
