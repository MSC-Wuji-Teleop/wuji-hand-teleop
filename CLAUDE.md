...
The hardware source of truth is [docs/spec/hardware_spec.md](docs/spec/hardware_spec.md): the rig's physical G1 is the **29-DoF** variant (confirmed on hardware 2026-09-01: all 14 arm joints, wrist pitch/yaw included, tracked in Stage B); `g1_wuji2_description` carries both `g1_29_wuji2*` and the secondary `g1_23_wuji2*` files. The upstream Tianji arm, HTC/SteamVR, and MANUS code was removed on 2026-08-25; what went and why is in [docs/deprecated/cleanup.md](docs/deprecated/cleanup.md). `src/camera/` is kept but unwired, pending the G1 head cameras.
...
- **Hardware replay**: the rig's robot is 29-DoF, and `arm_type=G1_29` (the default) drives DDS through the same `G1ArmController`; pose-IK stays G1_23-only. The spec_1 pipeline is built (offline `condition_clip` artifacts, gated `replay_publisher`, device FSMs + Layer-1 safety chains in both device nodes, supervisor with load gates/barrier/Layer-3 monitors/mcap logging; contracts in [docs/spec/spec_1_interfaces.md](docs/spec/spec_1_interfaces.md)). **The arm track ran on the rig 2026-09-01: Stage A (read-only + comm soak), Stage B (all 14 arm joints), and Stage C (sweep clip, all three arm scopes at 0.25x) all passed** — status and evidence pointers at the top of [docs/spec/spec_1_bringup.md](docs/spec/spec_1_bringup.md). The hand track has not run, and the Hand 2 mount adapter is still unconfirmed (vendor STL is a Hand v1 part; the `hand_dock` link the composed models grew on 2026-08-29 has never been measured against a physical part), which blocks combined stages and the measured flange→wrist transform TUITION.md §4 requires.
- The rig has **no dedicated e-stop**: the physical stop is the robot's remote damp command or main power, within reach during any powered run.
- Bundle clips are hardware no-gos as shipped (every `target_meta.json` says `real_robot_ready: false`; 9 clips carry single-frame wrist steps, see usage.md Stage E). Hardware runs use the sweep-test sample (`RobotSTAR_demos/sweep-test/`) instead; before any bundle clip runs, follow TUITION.md's staged sequence and pick from the batch audits — at least one sample failed its deployment audit (§7 names it) and several ship with `safe_timing_at_requested_scale: false`.
