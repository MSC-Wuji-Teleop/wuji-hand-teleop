...
- **Hardware replay**: the rig's robot is 29-DoF, and `arm_type=G1_29` (the default) drives DDS through the same `G1ArmController`; pose-IK stays G1_23-only. The spec_1 pipeline is built (offline `condition_clip` artifacts, gated `replay_publisher`, device FSMs + Layer-1 safety chains in both device nodes, supervisor with load gates/barrier/Layer-3 monitors/mcap logging; contracts in [docs/spec/spec_1_interfaces.md](docs/spec/spec_1_interfaces.md)) but container verification and the staged bring-up (Stages A-F) have not run. The Hand 2 mount adapter is still unconfirmed (vendor STL is a Hand v1 part; the `hand_dock` link the composed models grew on 2026-08-29 has never been measured against a physical part), which blocks combined stages and the measured flange→wrist transform TUITION.md §4 requires.
