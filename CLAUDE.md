...
- Hardware replay is validated (2026-09-05): the connection check, every safe clip one arm and one hand at a time, and the full both-arms both-hands replay all ran on the rig. `scripts/replay.sh --home` did not. `arm_type=G1_29` (the default) drives DDS through `G1ArmController`; pose-IK stays `G1_23`-only. The Hand 2 mount adapter is built and bolted on, with its orientation confirmed against the rig; the models carry it as a zero-thickness transform, so audit contact distances are short by the plate's real stack height ([docs/spec/hardware_spec.md](docs/spec/hardware_spec.md#mounting-adapter)).
