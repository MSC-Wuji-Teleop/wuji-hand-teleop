...
- `src/input_devices/`: `wuji_glove` (in-process `wuji_sdk` UDP, no topic hop), `pico_input` (chest-frame `PoseStamped` topics; also owns the PICO frame math in `transform_utils.py` + `config_loader.py`), `replay` (plays a prepared clip directory: waits for consumers, approaches frame 0, then named arm joint targets to the G1 node and named hand joints to the hand drivers at 100 Hz).
...
- `replay_publisher` reads a safe clip, waits for the selected consumers, approaches frame 0 from the measured pose, then publishes named `JointState` targets at 100 Hz (clip frames interpolated): arms to `g1_world_output` (`mode:=joint_replay arm_type:=G1_29`, interpolates one publish period behind), hand joints straight to the `starport_wuji_hand` drivers. The hand controller is not on this path.
...
- The bundle's **arm** joints assume the authors' hand mount, 90 deg from this rig's about the forearm axis (left +90, right -90 on `wrist_yaw_link`). `prepare_clip.py` records the source's `detected_hand_model` in `clip.json`; `tools/sanitize_clip.py` re-clocks the extracted wrist placement before its IK (`--wrist-clock`, gated on that key, `bundle` to force it on an older clip). A bundle clip is not filed as safe without that pass and a re-audit. Measurement: [docs/issues/wrist-clock-2026-09-11.md](docs/issues/wrist-clock-2026-09-11.md).
- There is no rehome. `scripts/replay.sh --home`, `tools/make_home_clip.py`, `capture_arm_pose` and `clips/home/` were removed on 2026-09-08, having never run in sim or on the rig; `PLAYABLE_PARENT_DIR_NAMES` is `("safe",)`. Do not re-propose one without new numbers: the measurement that settled it, where the committed clips actually leave the arms, is in [docs/spec/spec1_2.md](docs/spec/spec1_2.md#removing-the-rehome). After a clip, Ctrl-C hands the arms to the onboard controller from wherever they ended.
...
- Two host entry points, both unprivileged shells over the same graph. `scripts/replay.sh` is one clip per invocation with an exit-code contract, and is what tests and scripts use. `scripts/replay_interactive.py` is a session: the hand drivers start once and persist, the G1 container cycles per clip (stopping it is what ramps the `arm_sdk` weight 1 to 0), and Ctrl-C always tears down publisher, then G1, then drivers. Its prompt loop is `scripts/interactive/terminal.py`, which has no Docker or ROS in it and is tested under plain pytest (`python3 -m pytest scripts/tests/`). The session lists clips, starts one publisher, and stops it: it reads no robot state and judges no clip ([docs/spec/spec1_2.md](docs/spec/spec1_2.md)). Not yet run on hardware.
- Hardware replay is validated (2026-09-05): the connection check, every safe clip one arm and one hand at a time, and the full both-arms both-hands replay all ran on the rig. `arm_type=G1_29` (the default) drives DDS through `G1ArmController`; pose-IK stays `G1_23`-only. The Hand 2 mount adapter is built and bolted on, with its orientation confirmed against the rig; the models carry it as a zero-thickness transform, so audit contact distances are short by the plate's real stack height ([docs/spec/hardware_spec.md](docs/spec/hardware_spec.md#mounting-adapter)).
